...
- **Shake**: amplitud inicial 5 u, 40 pasos, 1 paso cada 24 ms (~1 s total), amplitud −1 cada 10 pasos; desplaza el *punto de mira* con `amp·(cos(paso), sin(paso))`. **CORRECCIÓN (verificada jugando DO 3D, ago-2026)**: NO dispara con daño normal — solo con el tipo de daño `"I"` (detonaciones tipo mina/kamikaze; el switch de `§_-j3H§` línea 79 lo exige) y con efectos cuyo XML declara `shakeScreen="true"`. El láser/cohete/plasma corriente jamás sacude la cámara. (La primera versión de este doc decía "cuando golpean al jugador" por una generalización del análisis; la prueba en vivo mandó.)
