...
1. **I1 — El contrato completo** (`mex-orbit-protocol`): extender `schema/messages.yaml` a los ~35 mensajes de `docs/protocolo-v1.md` §4; el generador aprende `repeated`, submensajes y `bool`; las pruebas del spike se extienden a un mensaje compuesto. *Observable: roundtrip byte-exacto de un `CollectResult` con drops.*
2. **I2 — La api mínima** (`mex-orbit-api`): ASP.NET sobre `mexorbit_dev`; `/auth/login` con Argon2id + game ticket Ed25519 + session token; semillas de cuenta (usuario + TestBot); registro tras flag. *Observable: `curl` de login devuelve ticket verificable.*
3. **I3 — El game server mínimo** (`mex-orbit-game-server`): wss + TLS dev, handshake con validación del ticket, sesión única con `SessionReplaced`, heartbeat, tick fijo 80 ms, `EnterMap` + spawns del Vex desde BD, `MoveIntent`→clamp→eco autoritativo. *Observable: un cliente de consola vuela por el mapa 1-1.*
